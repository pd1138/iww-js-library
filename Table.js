...
this.tbody = node;
this.buildRows = fuct;
this.clearTable = function( ){while(this.tbody.rows.length > 0){this.tbody.deleteRow(0);}};
...
if(this.tbody.rows.length > 0){this.clearTable();}
if(list.length === 0){
this.buildEmptyTable();
return;}
for(var i=0; i<list.length; i+=1){this.buildRows(list[i]);}
